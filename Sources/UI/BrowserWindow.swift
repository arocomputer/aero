    ...
    override func sendEvent(_ event: NSEvent) {
        if event.type == .leftMouseDown, event.clickCount == 1 {
            firstClickAllowsZoom =
                (windowController as? BrowserWindowController)?.allowsWindowZoom(at: event.locationInWindow) ?? false
        }
    ...
    override func performZoom(_ sender: Any?) {
        if let event = NSApp.currentEvent, event.clickCount >= 2 {
            guard firstClickAllowsZoom,
                (windowController as? BrowserWindowController)?.allowsWindowZoom(at: event.locationInWindow) == true
            else { return }
        }
