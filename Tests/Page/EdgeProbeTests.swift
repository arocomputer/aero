...
/// Runs the real probe script against a small page in a web view that is never put on screen, and
/// collects what it reports.
@MainActor private final class ProbedPage: NSObject, WKScriptMessageHandler {
    ...
    init(_ html: String) {
        ...
        super.init()
        // A window that is never shown counts as hidden, and the probe lets a hidden page wait. Only
        // the probe's own world is told otherwise; the page is left alone.
        ...
        EdgeChangeRouter.install(in: configuration.userContentController, handler: self)
        window.isReleasedWhenClosed = false
        window.contentView = webView
        webView.loadHTMLString(Self.base + html, baseURL: URL(string: "https://probe.example/"))
    }
...
@MainActor @Test func themeSwitchedByAClickIsNoticedWithoutScrolling() async {
    let page = ProbedPage("<style>header{background:#fff} .dark header{background:#000}</style><header></header><main></main>")
    #expect(await page.settles { $0 == "255,255,255" })
...
@MainActor @Test func colorsOutsideRGBNotationAreConvertedInsteadOfSnapshotted() async {
    let page = ProbedPage("<style>header{background:oklch(0.35 0.2 264)}</style><header></header><main></main>")
