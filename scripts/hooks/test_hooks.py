...
class HookTests(unittest.TestCase):
    def setUp(self):
        ...
        self.write('.swift-format', '{"version": 1, "indentation": {"spaces": 4}}\n')
    ...
    def test_staged_swift_is_checked_without_touching_unstaged_edits(self):
        path = self.write('Sources/file with spaces.swift', 'func main() {}\n')
        self.run_command('git', 'add', '.')
        path.write_text('func main( ){ }\n', newline='\n')
        result = self.check()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(path.read_text(), 'func main( ){ }\n')
        self.run_command('git', 'add', '.')
        path.write_text('func main() {}\n', newline='\n')
        result = self.check()
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('stage them again', result.stderr)
        self.assertEqual(self.run_command('git', 'show', ':Sources/file with spaces.swift').stdout, 'func main( ){ }\n')
    ...
    def test_existing_precommit_is_preserved_and_run(self):
        ...
        self.run_command('python3', str(INSTALL))
        self.assertEqual(self.run_command('git', 'config', '--worktree', '--get', 'aero.previousPreCommit').stdout.strip(), str(previous))
