...
# Aro is the system's WebKit plus its own code. A package dependency needs a decision, not a drive-by.
manifest = (root / "Package.swift").read_text(encoding="utf-8")
if re.search(r"\.package\s*\(", manifest):
    raise SystemExit("Package.swift: Aro has no package dependencies; see AGENTS.md before adding one")
